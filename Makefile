...
prepare: unmount
	sudo rm -f rpi0w-image rpi0w-image.*
	sudo dd if=/dev/zero of=rpi0w-image bs=1024 seek=3670015 count=1
	sudo sfdisk rpi0w-image < partioning
	sudo losetup -o 1048576 --sizelimit 535822336 /dev/loop1 rpi0w-image
	sudo losetup -o 536870912 --sizelimit 67108864 /dev/loop2 rpi0w-image
	sudo losetup -o 603979776 --sizelimit 469762048 /dev/loop3 rpi0w-image
	sudo losetup -o 1073741824 /dev/loop4 rpi0w-image
...
debootstrap:
	sudo debootstrap $(FOREIGN) --arch armel buster mnt \
...
compress:
	fakeroot xz -9 -k rpi0w-image
	sha512sum rpi0w-image.xz rpi0w-image > rpi0w-image.sha512
	gpg -ab rpi0w-image.sha512
